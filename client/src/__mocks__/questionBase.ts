...
export const generateRandomQuestions = (): Question[] => {
...
export const mockedGetQuestionsRequest: Promise<Question[]> = new Promise((res) => {
    const delay = Math.ceil(Math.random()) * 1000 + 2000;

    setTimeout(() => {
        res(generateRandomQuestions());
    }, delay);
});
